...
data:
  ...
  hooks:
    ...
    postStart: |-
      ...
      # Wait until client nodes are ready. This prevents data pods from crash looping and getting into a long backoff.
      echo "Waiting for client node..."
      # Use for loop to retry on connection failures. Elasticsearch image's curl doesn't have --retry-connrefused.
